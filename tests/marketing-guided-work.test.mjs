...
for(const [name,state,check,guardian,context,expected,kind] of [
 ...
 ['Guardian re-review','guardian_running','done','working',{guardian:{status:'started',retry_authorization:{event:'guardian_rereview_requested'}}},['done','working'],'review_again'],
...
test('pending human Guardian retry hides the prior verdict while retaining deterministic CHECK evidence',()=>{
 const w=work('changes_needed');const ctx={guardian:{status:'succeeded',recommendation:'needs_changes'}};
 const p=guidedPipeline(w,ctx,{pendingGuardianRetry:true});assert.equal(p.kind,'review_again');
 assert.deepEqual(p.stages.filter(s=>['CHECK','REVIEW'].includes(s.label)).map(s=>s.status),['done','working']);
});
