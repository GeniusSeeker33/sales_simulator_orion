...
export const fixture={userId:id(1),delay:0,packets:[],subscribe(fn){listeners.add(fn);return()=>listeners.delete(fn);},setUser(value){fixture.userId=value;authListeners.forEach(fn=>fn(value?'SIGNED_IN':'SIGNED_OUT',value?{user:{id:value}}:null));listeners.forEach(fn=>fn());}};
