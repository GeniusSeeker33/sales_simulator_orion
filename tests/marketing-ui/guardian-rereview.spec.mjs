...
test('human can re-review unchanged revision 3 at cycle 1/3, see another recommendation and then approve',async({page})=>{
  ...
  const setup=await read(page);const campaign=setup.campaigns.find(c=>c.name==='Guardian re-review campaign');
  const policyResponse=await page.request.post('/__marketing_rpc',{headers:{'x-test-user':'2'},data:{name:'save_marketing_policy',args:{p_workspace:setup.workspace.id,p_campaign:campaign.id,p_expected_set:null,p_rules:[{constraint_type:'no_unverified_comparative_claim'}]}}});expect((await policyResponse.json()).error).toBeNull();
  await open(page,`/marketing/campaigns/${campaign.id}`);
  ...
  const legacyRun=(await(await page.request.post('/__marketing_rpc',{headers:{'x-test-user':'2'},data:{name:'read_marketing_agent_run',args:{p_workspace:before.workspace.id,p_run:o.active_run_id}}})).json()).data;
  expect(legacyRun.guardian_evidence.retry_authorization).toBeNull();expect(legacyRun.output_metadata.deterministic_qa.some(q=>q.passed===false&&q.matched_text==='best')).toBe(true);
  ...
  await card.getByText('Advanced technical history',{exact:true}).click();await card.getByText('Workflow details & actions',{exact:true}).click();await expect(card.getByRole('button',{name:/Send.*Creator|Send Revision to Creator/})).toHaveCount(0);const evidence=card.getByRole('region',{name:'Guardian retry authorization'});
