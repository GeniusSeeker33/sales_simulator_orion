...
test('explicit agent actions feed the governed workflow and visible run outcomes', async ({ page }) => {
  ...
  await page.locator('summary').filter({ hasText: 'creator · succeeded' }).click();
  const assetPath = await page.getByText('Outcome asset:').getByRole('link').getAttribute('href');
  ...
  await expect(page.getByRole('link', { name: 'Approvals', exact: true })).toHaveAccessibleDescription('1 unresolved items: Human action required');
  await expect(page.getByRole('link', { name: 'Agents', exact: true })).toHaveAccessibleDescription('3 unresolved items: Human action required');
  await page.getByRole('link', { name: 'Overview', exact: true }).click();
  const attention = page.getByRole('region', { name: 'Needs Your Attention' });
  await expect(attention.locator('li')).toHaveCount(4);
  await expect(attention.locator('li').first()).toContainText('Human action required');
  await expect(attention.locator('li').last()).toContainText('Review recommended');
  await attention.screenshot({ path: 'test-results/marketing-ui/attention-desktop.png' });
  await page.setViewportSize({ width: 390, height: 844 });
  await attention.scrollIntoViewIfNeeded();
  await attention.screenshot({ path: 'test-results/marketing-ui/attention-mobile.png' });
  ...
  // Only a human requests changes, and Creator must return for a new human decision.
  ...
  await page.getByLabel('Review notes / requested changes').fill('Make the guided demo benefit explicit.');
  ...
  await page.goto(`/tests/marketing-ui/index.html?user=1&path=${encodeURIComponent(assetPath)}`);
  await page.getByLabel('Supplemental revision instructions (optional)').fill('Keep it concise.');
  await page.getByLabel('Submit revised asset for human review').check();
  await page.getByRole('button', { name: 'Send requested changes to Creator', exact: true }).click();
  await expect(page.getByText('Campaign:', { exact: false }).filter({ hasText: 'Asset revision 5' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Record human decision' })).toHaveCount(0);
  await page.locator('summary').filter({ hasText: 'creator · succeeded · Revise human requested changes' }).click();
  await page.getByText('Revision handoff evidence', { exact: true }).click();
  await expect(page.getByText('Human requested changes: Make the guided demo benefit explicit.', { exact: true })).toBeVisible();
  await expect(page.getByRole('link', { name: 'Inspect prior Guardian assessment' })).toBeVisible();
  await page.goto('/tests/marketing-ui/index.html?user=2&path=/marketing/approvals');
  await page.locator('summary').filter({ hasText: 'Creator browser draft · social copy · in review' }).click();
  await page.getByRole('button', { name: 'Record human decision' }).click();
  await expect(page.locator('summary').filter({ hasText: 'Creator browser draft · social copy · in review' })).toHaveCount(0);
  await page.goto(`/tests/marketing-ui/index.html?user=3&path=${encodeURIComponent(assetPath)}`);
  await expect(page.getByRole('button', { name: 'Send requested changes to Creator' })).toHaveCount(0);
