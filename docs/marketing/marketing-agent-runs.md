...
| Save/submit text assets | No | Yes | Yes | Creator: new draft or explicitly authorized revision |
...
- Creator: `name`, supported `asset_type`, nonempty `content`. The type must match the initiating request. The human chooses save-only or save-and-submit. The completion RPC invokes the existing private agent asset pathway, fixes `created_via=agent`, sets `assets.agent_run_id`, and preserves run attribution in workflow history. Agent-created assets with a current human change request can now be revised through the [governed revision handoff](marketing-attention-and-revisions.md); historical versions remain immutable.
- Guardian: `summary`, `recommendation` (`ready_for_human_review` or `needs_changes`), categorized findings with severity and an explicit correction-required flag. Deterministic QA checks content, supported type, audience/channel presence, destination/action wording and unresolved placeholders. [Guardian QA calibration](guardian-qa-calibration.md) documents semantic-review hints and recommendation normalization. Any failed deterministic check forces `needs_changes`. These checks are intentionally limited; semantic claims and suitability still require human review. Recommendations do not change asset approval, record review decisions or publish.
...
Marketing → Agents shows the most recent 50 workspace runs and expandable outcomes, timestamps, identity, usage and cost availability. Attention signals and Creator revision handoffs are described in [Human attention and governed revisions](marketing-attention-and-revisions.md). Campaign pages provide Ask Strategist, Create draft with Creator and Run Guardian QA, with required human instructions and explicit Creator submission/Guardian asset controls. Refresh is manual. Guardian results describe their recorded revision and do not replace the Approvals workflow.
