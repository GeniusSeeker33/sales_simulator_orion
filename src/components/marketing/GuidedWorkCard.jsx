...
export default function GuidedWorkCard({ data, work, context, onRefresh, children }) {
  ...
  const { stages, current: stage, kind } = guidedPipeline(work, context, { pendingGuardianRetry: busy && pendingAction === 'retry_guardian' });
  const reviewingAgain = kind === 'review_again';
  const rereview = context.guardian_retry?.kind === 'rereview';
  const actions = reviewingAgain ? [] : context.actions || [];
  ...
  async function command(action, decision) {
    ...
    finally { try { await onRefresh(); } catch { setError('Refresh failed. Inspect current work before retrying.'); } setBusy(false); setPendingAction(null); }
  }
  return <article className="card marketing-guided-work" id={`work-${o.id}`}>
    ...
    <ol className="marketing-stage-pipeline" aria-label="Guided workflow pipeline">{stages.map(({ label: name, status }) => <li key={name} className={`stage-${status}`} aria-current={stage === name ? 'step' : undefined}><strong>{name} <span aria-hidden="true">{({ done: '✓', action: '!', working: '↻', failed: '✕', inconsistent: '⚠', technical: '⚠', pending: '—', unknown: '?' })[status]}</span></strong><small>{stage === name ? 'Current step · ' : ''}{({ done: 'Completed', action: 'Human decision needed', working: name === 'REVIEW' && reviewingAgain ? 'Guardian reviewing again' : 'In progress', failed: name === 'CHECK' ? 'Policy issue' : 'Could not complete', inconsistent: 'Guardian review inconsistent', technical: 'Guardian could not complete', pending: 'Not reached', unknown: 'Needs inspection' })[status]}</small></li>)}</ol>
    <p><strong>Step {Math.max(1, stages.findIndex(s => s.label === stage) + 1)} of {stages.length}: {stage === 'DONE' ? 'Agent work complete' : kind === 'guardian_inconsistent' ? 'Guardian review needs retry' : kind === 'guardian_technical' ? 'Guardian could not complete' : kind === 'guardian_changes' ? 'Guardian recommends changes' : kind === 'ready' ? 'Ready for your review' : reviewingAgain ? 'Guardian reviewing again' : stage === 'REVIEW' ? 'Guardian is reviewing' : stage === 'PLAN' ? 'Review the execution plan' : kind === 'policy_issue' ? 'Revision blocked by Marketing Policy' : stage === 'CHECK' ? 'Checking Marketing Policy' : 'Creating your draft'}</strong></p>
    <p role={reviewingAgain ? 'status' : undefined}>{stage === 'DONE' ? 'The agent workflow is complete. You decide when the task itself is done.' : reviewingAgain ? 'Guardian is reviewing the same draft again. The content has not changed.' : plainWorkReason(context.reason)}</p>
    ...
    {rereview && actions.includes('retry_guardian') && <p id={`rereview-help-${o.id}`}>Run Guardian again on this same draft without changing the content or using a revision cycle.</p>}
    <fieldset className="marketing-fieldset" disabled={busy}>
      {!editing && <div className="marketing-actions">
        ...
        {actions.includes('request_changes') && <button type="button" className={kind === 'guardian_changes' ? 'btn-primary' : 'btn-secondary'} onClick={() => { setRules(editableRules(current?.constraints)); setEditing(true); }}>{kind === 'guardian_changes' ? 'Request Changes & Send for Revision' : recovery ? 'Continue this work' : 'Request Changes'}</button>}
        {actions.includes('retry_guardian') && <button type="button" className={rereview ? 'btn-secondary' : 'btn-primary'} aria-describedby={rereview ? `rereview-help-${o.id}` : undefined} onClick={() => command('retry_guardian')}>{rereview ? 'Retry Guardian review' : 'Retry Guardian'}</button>}
      ...
      {editing && !reviewingAgain && <form onSubmit={e => { e.preventDefault(); command('review', 'request_changes'); }}>
      ...
      {busy && !reviewingAgain && <p role="status">{editing ? "Revision requested. Creator is revising your content; policy checks and Guardian review follow before your next decision." : "Executing your authorized action…"}</p>}
    </fieldset>
    {error && <p role="alert" className="marketing-error">{error}</p>}
    <details open={advanced} onToggle={e => setAdvanced(e.currentTarget.open)}><summary>Advanced technical history</summary>{advanced && <><GuardianEvidence evidence={context.guardian} />{actions.includes('retry_guardian') && <button type="button" disabled={busy} onClick={() => command('retry_guardian')}>{rereview ? 'Retry Guardian review' : 'Retry Guardian'}</button>}{!reviewingAgain && children}</>}</details>
  </article>;
}
