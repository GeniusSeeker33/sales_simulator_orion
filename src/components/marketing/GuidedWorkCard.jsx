...
export default function GuidedWorkCard({ data, work, context, onRefresh, children }) {
  ...
  return <article className="card marketing-guided-work" id={`work-${o.id}`}>
    ...
    <ol className="marketing-stage-pipeline" aria-label="Guided workflow pipeline">{stages.map(({ label: name, status }) => <li key={name} className={`stage-${status}`} aria-current={stage === name ? 'step' : undefined}><strong>{name} <span aria-hidden="true">{({ done: '✓', action: '!', working: '↻', failed: '✕', technical: '⚠', pending: '—', unknown: '?' })[status]}</span></strong><small>{stage === name ? 'Current step · ' : ''}{({ done: 'Completed', action: 'Human decision needed', working: 'In progress', failed: name === 'CHECK' ? 'Policy issue' : 'Could not complete', technical: 'Guardian could not complete', pending: 'Not reached', unknown: 'Needs inspection' })[status]}</small></li>)}</ol>
    <p><strong>Step {Math.max(1, stages.findIndex(s => s.label === stage) + 1)} of {stages.length}: {stage === 'DONE' ? 'Agent work complete' : kind === 'guardian_technical' ? 'Guardian could not complete' : kind === 'guardian_changes' ? 'Guardian recommends changes' : kind === 'ready' ? 'Ready for your review' : stage === 'REVIEW' ? 'Guardian is reviewing' : stage === 'PLAN' ? 'Review the execution plan' : kind === 'policy_issue' ? 'Revision blocked by Marketing Policy' : stage === 'CHECK' ? 'Checking Marketing Policy' : 'Creating your draft'}</strong></p>
